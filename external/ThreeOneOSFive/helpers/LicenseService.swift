...
final class LicenseService: ObservableObject {
    ...
    static let apiBaseURL = "https://moddtoolsadminapi.netlify.app"pp"
    ...
    private let stateKey = "moodtools_license_state"
    private let deviceID: String = {
        if let saved = UserDefaults.standard.string(forKey: "moodtools_device_id") {
            return saved
        }
        let id = UIDevice.current.identifierForVendor?.uuidString ?? UUID().uuidString
        UserDefaults.standard.set(id, forKey: "moodtools_device_id")
        return id
    }()
